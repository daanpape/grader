...
class rapportenDAO {
    ...
    public static function getStudentGroupTeacherByCourseID($courseid) {
        try {
            $conn = Db::getConnection();
            $stmt = $conn->prepare("SELECT *FROM course_studentlist_teacher_rapport LEFT JOIN studentlist_rapport ON course_studentlist_teacher_rapport.studentlist = studentlist_rapport.id LEFT JOIN users ON course_studentlist_teacher_rapport.teacher = users.id WHERE  course_studentlist_teacher_rapport.course = 1");
