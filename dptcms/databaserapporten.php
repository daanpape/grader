...
class rapportenDAO {
    public static function getAllCourses($start, $count) {
        try {
            ...
            $stmt = $conn->prepare("SELECT * FROM course_rapport JOIN    WHERE active = '1' LIMIT :start,:count");
...
");
 ...
 SELECT  (select name from module_rapport where course=:id) as name,( select description from module_rapport where course=:id) as description"
