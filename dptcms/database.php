...
class ClassDAO
{
    ...
    public static function insertProject($courseid, $code, $name, $description, $nrOfAssessing, $projectPercent, $documentPercent)
    {
        try {
            $conn = Db::getConnection();
            $stmt = $conn->prepare("INSERT INTO project (code, name, description, course, nrOfAssessing, project_percent, document_percent) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute(array($code, $name, $description, $courseid, $nrOfAssessing, $projectPercent, $documentPercent));
    ...
    public static function updateProject($id, $code, $name, $description, $nrOfAssessing, $documentPercent, $projectPercent)
    {
        try {
            $conn = Db::getConnection();
            $stmt = $conn->prepare("UPDATE project SET code = ?, name = ?, description = ?, nrOfAssessing = ?, document_percent = ?, project_percent = ? WHERE id = ?");
            $stmt->execute(array($code, $name, $description, $nrOfAssessing, $documentPercent, $projectPercent, $id));
            return true;
        } catch (PDOException $err) {
    ...
    public static function getUserDataForDocument($projectid, $userid)
    {
        try {
            $conn = Db::getConnection();
            $stmt = $conn->prepare("SELECT assess_documents.id, assess_documents.document, assess_documents.student, assess_documents.project, assess_documents.not_submitted, documenttype.nr_documents FROM assess_documents JOIN documenttype ON assess_documents.document = documenttype.id WHERE project = ? AND student = ?");
            $stmt->execute(array($projectid, $userid));
            $dataFromDb = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public static function saveProjectDocuments($projectId, $documents)
    {
        ...
        foreach($data as $document)
        {
            if($document->id == 0)
            {
                $stmt = $conn->prepare("INSERT INTO documenttype (description,point_type,weight,project,locked, nr_documents) VALUES (?,?,?,?,?,?)");
                $stmt->execute(array($document->description,$document->pointType, $document->weight,$projectId,$document->locked,$document->nrDocuments));
            }
            else
            {
                $stmt = $conn->prepare("UPDATE documenttype SET description = ?, point_type = ?, weight = ?, locked = ?, nr_documents = ? WHERE id= ?");
                $stmt->execute(array($document->description, $document->pointType, $document->weight, $document->locked, $document->nrDocuments,$document->id));
            }
        }
    ...
    public static function getStudentListWithNrOfAssessed($pid)
    {
        try {
            ...
            foreach($students as $student)
            {
                ...
                if (intval($nr[0]) <= intval($data)){
                    $studentData->assessCompleted = 'COMPLETED';
                } else if (intval($data) > 0 && intval($data) != intval($nr[0])){
