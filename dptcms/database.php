    ...
    public static function saveProjectRules($id, $projectrules)
    {
        try
        {
            ...
            foreach ($data as $rule) {
                if(!isset($rule->id)) {
                    $stmt = $conn->prepare("INSERT INTO rules (project, name, subject, action, operator, value, result) VALUES (?,?,?,?,?,?)");
                    $stmt->execute(array($id, $rule->name, "Test", $rule->action->name, $rule->operator, (int)$rule->value, (int)$rule->result));
                    $count++;
                }
                else
                {
                    $stmt = $conn->prepare("UPDATE rules SET project=?, name=?, action=?, operator=?, value=?, result=? WHERE id=?");
                    $stmt->execute(array($id, $rule->name, $rule->action->name, $rule->operator, (int)$rule->value, (int)$rule->result, (int)$rule->id));
                }
            }
            return $count;
        }
