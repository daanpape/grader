...
function loadCoupleDropdown() {
    $.getJSON('/api/studentlists/' + viewModel.userId, function(data) {
        $.each(data, function(i, item) {
            $("#ddlLists").append('<li class="li-wide" role="presentation"><a class="studentListItem" role="menuitem" tabindex="-1" href="#" id="\'dropdownitem-\'' + item.id + '><span>' + item.name + '</span></a> </li>')
        });
    });
